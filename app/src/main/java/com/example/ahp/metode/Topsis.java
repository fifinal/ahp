...
//        TreeMap<String, ArrayList<Double>> normalisedSitesMatrix = calculateNormalisedFuzzyMatrix(sitesMatrix);
        ...
        System.out.println("**********************************");
        System.out.println("Final Ranking:"+ccStar.entrySet().size());

        Data.finalRanking=new ArrayList<>();
        for (Map.Entry<Double, String> entry: ccStar.entrySet()){
            Map<String,String> map=new HashMap<>();
            map.put("key",entry.getValue());
            map.put("value",Data.df.format(entry.getKey()));
            Data.finalRanking.add(map);
            System.out.println(entry.getValue() + ": " + Data.df.format(entry.getKey()));
        }
    }
    ...
    private TreeMap<String, ArrayList<Double>> calculateWeightedFuzzyMatrix(TreeMap<String, ArrayList<Double>> sitesMatrix){
        ...
        for (Map.Entry<String,ArrayList<Double>> entry: sitesMatrix.entrySet()) {
            ...
            for (int i = 0; i < entry.getValue().size(); i++) {
                weightedMatrix.add(entry.getValue().get(i) * Data.ahpWeights[j]);
                if (j%3 == 0)
                    j++;
            }
        ...
        for (Map.Entry<String, ArrayList<Double>> entry: normalisedWeightedFuzzyMatrix.entrySet()){
            ...
            for (int j=0; j< entry.getValue().size(); j++) {
                ...
                System.out.print(Data.df.format(d) + ", ");
            }
    ...
    private TreeMap<String, Double> calculateDistance(TreeMap<String, ArrayList<Double>> sitesMatrix, boolean ideal) {
        ...
        for (Map.Entry<String,ArrayList<Double>> entry: sitesMatrix.entrySet()) {
            ...
            for (int i = 0; i < entry.getValue().size(); i = i + 3) {
                ...
                if (ideal) { // For D+
                    if (Data.costCriteria[i/3]) { // cost value for price criterion
//                        dValue += distance.compute(fuzzyValues, Data.antiIdealSolution) * (1.0/3.0);
                        dValue += Math.sqrt((Math.pow(entry.getValue().get(i) - Data.antiIdealSolution[0], 2) +
                                Math.pow(entry.getValue().get(i+1) - Data.antiIdealSolution[1], 2) +
                                Math.pow(entry.getValue().get(i+2) - Data.antiIdealSolution[2], 2)) * (1.0/3.0));
                    } else {
//                        dValue += distance.compute(fuzzyValues, Data.idealSolution) * (1.0/3.0);
                        dValue += Math.sqrt((Math.pow(entry.getValue().get(i) - Data.idealSolution[0], 2) +
                                Math.pow(entry.getValue().get(i+1) - Data.idealSolution[1], 2) +
                                Math.pow(entry.getValue().get(i+2) - Data.idealSolution[2], 2)) * (1.0/3.0));
                    }
                } else { // For D-
                    if (Data.costCriteria[i/3]) { // cost value for price criterion
//                        dValue += distance.compute(fuzzyValues, Data.idealSolution) * (1.0/3.0);
...
//                        dValue += distance.compute(fuzzyValues, Data.antiIdealSolution) * (1.0/3.0);
            ...
            System.out.println(" for " + entry.getKey() + " is: " + Data.df.format(dValue));
        }
    ...
    private TreeMap<Double, String> calculateRelativeCloseness(TreeMap<String, Double> dPlusList, TreeMap<String, Double> dMinusList) {
        ...
        System.out.println(" size "+dPlusList.entrySet().size());
